...
void PionsNeg(Char_t path_file[100],Char_t jobid[100]){

  mOutputFileName = Form("%s_%s.root",histname[3].c_str(),jobid);
  ...
  for(int i=0;i<nCuts;i++){
    for(int c=0;c<9;c++){
      sprintf(buf,"%s_%s_%d_%d",histname[3].c_str(),energyname.c_str(),i,c);

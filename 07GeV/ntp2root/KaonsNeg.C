...
void KaonsNeg(Char_t path_file[100],Char_t jobid[100]){

  mOutputFileName = Form("%s_%s.root",histname[5].c_str(),jobid);
  ...
  for(int i=0;i<nCuts;i++){
    for(int c=0;c<9;c++){
      sprintf(buf,"%s_%s_%d_%d",histname[5].c_str(),energyname.c_str(),i,c);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(( mass2kaonsMinDef < mMass2T) && (mMass2T < mass2kaonsMaxDef) )  iCut[4][0] =1;
    if(( mass2kaonsMinMin < mMass2T) && (mMass2T < mass2kaonsMaxMin) )  iCut[4][1] =1;
    if(( mass2kaonsMinMax < mMass2T) && (mMass2T < mass2kaonsMaxMax) )  iCut[4][2] =1;
    ...
    // Momentum
    if(mMomentumT <= kaonsPMaxDef && kaonsPtMinDef <= mPtT )	 	iCut[6][0] =1;
    if(mMomentumT <= kaonsPMaxMin && kaonsPtMinMin <= mPtT )		iCut[6][1] =1;
    if(mMomentumT <= kaonsPMaxMax && kaonsPtMinMax <= mPtT )		iCut[6][2] =1;

...
TGraphErrors* gPoint3;
TGraphErrors* gPoint4;
TGraphErrors* gPoint5;
TGraphErrors* gPoint6;
...
void Plot(){
  TCanvas *c = new TCanvas("c", "",216,211,1200,500);
  ...
  TH1F *hframe__1 = new TH1F("hframe__1","",1200,-1,36);
  ...
  gPoint3->Draw("Psame9");
  gPoint4->Draw("p[]same9");


  gPoint5->Draw("Psame9");
  gPoint6->Draw("p[]same9");
...
void GetSys(){
  ...
  for(int f=1;f<31;f++){
    ...
    if(f==25 || f==26 ) continue;
    if(f==9 || f==10 ){
      rms2 += (0.5 * ( (yData[0]-yData[f]) * (yData[0]-yData[f]) ) + ( (yData[0]-yData[f]) * (yData[0]-yData[f]) ) );
      f++;
      continue;
    }
    if(f==17 || f==18 ){
      ...
      rms2 += (0.5 * ( (yData[0]-yData[f]) * (yData[0]-yData[f]) ) + ( (yData[0]-yData[f+1]) * (yData[0]-yData[f+1]) ) );

    f++;
  }

  
  ype[0] =   TMath::Sqrt(rms2) ;
  ...
  ype[0]=eYData[0];
  gPoint3 = new TGraphErrors(1,xp,yp,xpe,ype);
  gPoint3->SetMarkerStyle(20);
  gPoint3->SetMarkerSize(1.2);
  gPoint3->SetMarkerColor(2);
  ...
  ype[0] =   TMath::Sqrt(rms2) ;
  gPoint4 = new TGraphErrors(1,xp,yp,xpe,ype);
  ...
  ype[0]=eYData[0];
  gPoint5 = new TGraphErrors(1,xp,yp,xpe,ype);
  gPoint5->SetMarkerStyle(20);
  gPoint5->SetMarkerSize(1.2);
  gPoint5->SetMarkerColor(2);
  ...
  ype[0] =   TMath::Sqrt(rms2) ;
  gPoint6 = new TGraphErrors(1,xp,yp,xpe,ype);
...
void Fit(){
  ...
  forFit->Close();
  gFlag = new TGraphErrors(31,xx,yData,exx,eYData);
...
void GetPlots(){
  ...
  for(int f=0;f<31;f++){
    for(int c=0;c<9;c++){
      sprintf(buf,"%s_%d_%d","pV1L_19GeV",f,c);

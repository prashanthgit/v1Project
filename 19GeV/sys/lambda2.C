...
void Plot(){
  TCanvas *c = new TCanvas("c", "",216,211,830,300);
  ...
  hframe__1->GetXaxis()->SetTitleSize(0.12);
  hframe__1->GetXaxis()->SetTickLength(0.02);
  hframe__1->GetXaxis()->SetTitleOffset(0.8);
  ...
  hframe__1->GetYaxis()->SetTitleOffset(0.4);
...
void GetSys(){
  for(int b=1;b<11;b++){
    ...
    for(int f=1;f<31;f++){
      ...
      if(f==18 )continue;
      if(f==9 || f==17){
	rms += 2* ( ( pV1Data[0]->GetBinContent(b) -  pV1Data[f]->GetBinContent(b) ) * ( pV1Data[0]->GetBinContent(b) -  pV1Data[f]->GetBinContent(b) ) ); 
	continue;
      }

...
Bool_t StStrangeMaker::isBadRun(StMuEvent* ev){
 ...
 Int_t RunId      = ev->runNumber();
  Int_t badrun[35] ={15053027, 15053028, 15053029, 15053034, 15053035, 15053052, 15053054,
    15053055, 15054053, 15054054, 15055018, 15055137, 15056117, 15057055,
    15057059, 15058006, 15058011, 15058021, 15059057, 15059058, 15061001,
    15061009, 15062006, 15062069, 15065012, 15065014, 15066070, 15068013,
    15068014, 15068016, 15068018, 15069036, 15070008, 15070009, 15070010
  };

  for(int i=0;i<35;i++){if(RunId == badrun[i]) return kTRUE;}
...
Int_t StStrangeMaker::centrality(StMuEvent* ev){
  ...
  */
  ...
  //centrality from refmult from Daniel B.
  Int_t CentralityID = -1;
  Int_t refMult = ev->refMult() ;
  if(refMult >= 5 && refMult < 11)         CentralityID = 0; //70-80%
  else if(refMult >= 11 && refMult < 20)   CentralityID = 1; //60-70%
  else if(refMult >= 20 && refMult < 36)   CentralityID = 2; //50-60%
  else if(refMult >= 36 && refMult < 59)   CentralityID = 3; //40-50%
  else if(refMult >= 59 && refMult < 93)   CentralityID = 4; //30-40%
  else if(refMult >= 93 && refMult < 138)  CentralityID = 5; //20-30%
  else if(refMult >= 138 && refMult < 200) CentralityID = 6; //10-20%
  else if(refMult >= 200 && refMult < 239) CentralityID = 7; //5-10%
  else if(refMult >= 239)                  CentralityID = 8; //0-5%
